...
forval dataset_no = 1/8 {
	...
	* Create adjusted retirement ages 
	adjust_retage, dset("`wvsfx'")
	
	* Prepare the discount rate datasets to merge in 
	...
	* Prepare the data for merging in discount rates 
	get_approx_retiredist, dset("`wvsfx'")
	gen single = (!inlist(dvmrdf`wvsfx', 1, 2, 7, 8))
	
	* Prepare the annuity rates dataset to merge in 
	preserve 
	...
	* Merge in the discount rates and annuity factors
	forval i = 1/5 {
	...
	* And also add in the SCAPE rate 
	gen scpe_rate = .
	...
	* Run pension code with relevant discount rates 
